...
# Method: Query-aware Adaptive Chunk Size (QACS)
def qacs_chunk(html_text: str, query: str, model: SentenceTransformer, fine_size: int = 128, coarse_size: int = 512, sentence_batch_size: int = 64) -> list:
    """
    Query-aware Adaptive Chunk Size (QACS):
    - Split document into sentences (HTML-aware text extraction).
    - Compute sentence embeddings and similarity to the query.
    - Build chunks by using smaller chunk size in high-similarity regions (fine_size)
      and larger chunk size in low-similarity regions (coarse_size).
    ...
    if not sentences:
        ...
        return [" ".join(sentences[i:i+coarse_size]) for i in range(0, len(sentences), coarse_size)]

    # Encode query once
    q_vec = model.encode(query, convert_to_tensor=True, show_progress_bar=False)

    # Batch-encode sentences for efficiency
    sent_emb = model.encode(sentences, convert_to_tensor=True, batch_size=sentence_batch_size, show_progress_bar=False)
    sims = util.cos_sim(q_vec, sent_emb)[0].cpu().numpy().tolist()

    # Determine adaptive window per sentence
    # Use a percentile to define what is "high similarity"
    if sims:
        thresh = float(np.percentile(np.array(sims), 66))
    else:
        thresh = 0.0

    chunks = []
    cur_words = []
    cur_cap = coarse_size  # default to coarse

    for sent, sim in zip(sentences, sims):
        words = sent.split()
        # switch capacity depending on similarity
        target_cap = fine_size if sim >= thresh else coarse_size
        # If capacity setting changes and we already have content, flush
        if cur_words and target_cap != cur_cap:
            chunks.append(" ".join(cur_words))
            cur_words = []
        cur_cap = target_cap

        # Append while respecting capacity
        if len(cur_words) + len(words) > cur_cap and cur_words:
            chunks.append(" ".join(cur_words))
            cur_words = []
    ...
    if not chunks:
        ...
        for i in range(0, len(words), coarse_size):
            ch = " ".join(words[i:i+coarse_size]).strip()
            if ch:
                chunks.append(ch)
    return chunks
...
def evaluate_method(model, dataset_path, chunking_strategy, method_name, **chunk_params):
    ...
    with open(dataset_path, "r", encoding="utf-8") as f:
        for line in tqdm(f, desc=f"Evaluating {method_name}"):
            ...
            try:
                if method_name == "qacs":
                    chunks = chunking_strategy(html_text, question, model, **chunk_params)
...
print("="*70)
print("Running Chunking Strategy Comparison (per dataset)")
print("="*70)
...
# Run comparison per dataset
all_results_rows = []
for ds_name, ds_path in DATASETS.items():
    print(f"\nDataset: {ds_name} -> {ds_path}")
    ds_results = {}
    for strategy_name, (strategy_func, params) in strategies_map.items():
        print(f"\n{'='*70}")
        print(f"Chunking Strategy: {strategy_name.upper()} (dataset: {ds_name})")
        print(f"{'='*70}")
        
        ds_results[strategy_name] = evaluate_method(
            model=model,
        ...
        print(
            ...
            f"nDCG: {ds_results[strategy_name].get('ndcg', 0.0):.4f}, "
            f"Avg #chunks: {ds_results[strategy_name].get('avg_num_chunks', 0.0):.2f}"
        )
    # assemble rows with dataset column
    for strat, metrics in ds_results.items():
        row = {"dataset": ds_name, "method": strat}
        row.update(metrics)
        all_results_rows.append(row)
    # Save per-dataset CSV
    df_ds = pd.DataFrame(ds_results).T
    df_ds.to_csv(f"chunking_comparison_results_{ds_name}.csv")
    print(f"Saved per-dataset results to chunking_comparison_results_{ds_name}.csv")

print("\n" + "="*70)
print("All experiments completed across datasets!")
print("="*70)
...
print("\n=== Chunking Strategy Comparison Results (combined) ===")
print(df_results.to_string())

# Save combined CSV
df_results.to_csv("chunking_comparison_results.csv", index=False)
print("\nCombined results saved to chunking_comparison_results.csv")
...
# Per-dataset rankings by Recall@10
print("\n" + "="*70)
print("PERFORMANCE COMPARISON (per dataset)")
print("="*70)
...
for ds_name in DATASETS.keys():
    ...
    if df_ds.empty:
        continue
    sorted_by_recall = df_ds.sort_values(by="recall@10", ascending=False)
    print(f"\nRanking by Recall@10 for dataset: {ds_name}")
    for i, (_, row) in enumerate(sorted_by_recall.iterrows(), 1):
        print(f"  {i}. {str(row['method']).upper().replace('_', ' ')}: Recall@10={row['recall@10']:.4f}, MRR={row['mrr']:.4f}")

    # Best vs Worst comparison
    best = sorted_by_recall.iloc[0]
    worst = sorted_by_recall.iloc[-1]
    recall_improvement = ((best['recall@10'] - worst['recall@10']) / worst['recall@10'] * 100) if worst['recall@10'] > 0 else 0
    mrr_improvement = ((best['mrr'] - worst['mrr']) / worst['mrr'] * 100) if worst['mrr'] > 0 else 0
    print(f"\n🏆 Best Strategy ({ds_name}): {str(best['method']).upper().replace('_', ' ')}")
